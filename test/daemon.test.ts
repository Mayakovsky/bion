...
describe('daemon tick + heartbeat (E1)', () => {
  it('a tick drains durable intents and writes a fresh heartbeat', async () => {
    const path = hbPath()
    const hb = await tick(7, { heartbeatPath: path, watchGit: false, watchTests: false })
  ...
  it('the onTick hook runs (Auto Mode / usage wire in here)', async () => {
    const path = hbPath()
    let ran = 0
    await tick(1, { heartbeatPath: path, watchGit: false, watchTests: false, onTick: async () => { ran++ } })
    expect(ran).toBe(1)
  })

  it('isDaemonAlive reflects heartbeat freshness', async () => {
    const path = hbPath()
    const hb = await tick(1, { heartbeatPath: path, watchGit: false, watchTests: false })
